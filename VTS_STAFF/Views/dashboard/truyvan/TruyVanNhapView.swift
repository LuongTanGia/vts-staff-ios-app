...
struct TruyVanNhapView: View {
    ...
    init(fromDate: Date, toDate: Date) {
        _viewModel = StateObject(wrappedValue: TruyVanNhapViewModel(fromDate: fromDate, toDate: toDate))
    }
...
#Preview {
    RouterView { _ in
        TruyVanNhapView(fromDate: Date(), toDate: Date())
    }
}
