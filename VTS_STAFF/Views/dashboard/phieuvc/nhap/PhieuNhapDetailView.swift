...
struct PhieuNhapDetailView: View {
    ...
    private var galleryItems: [VTSPhotoGalleryItem] {
        [
            VTSPhotoGalleryItem(id: 1, title: "Ảnh 1 - Phieu nhập 1", image: hinh01, ocrText: hinh01Text),
            VTSPhotoGalleryItem(id: 2, title: "Ảnh 2 - Phiếu nhập 2", image: hinh02, ocrText: hinh02Text)
        ]
    }
    ...
    private var headerTitleDisplay: String {
        if viewModel.isNew {
            return "Chuyến hàng nhận mới"
        } else if isEditMode {
            return "Cập nhật chuyến hàng nhận"
        } else {
            return "Thông tin chuyến hàng nhận"
        }
    }
    ...
    private func saveVoucher() async {
        ...
        do {
            if viewModel.isNew {
                ...
                let existingDetails: TPhieuvc_Nhap_DanhSach? = if case .success(let d) = viewModel.state { d } else { nil }
