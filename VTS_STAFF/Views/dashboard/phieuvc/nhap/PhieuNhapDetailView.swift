...
struct PhieuNhapDetailView: View {
    ...
    var body: some View {
        ...
        .customToolbar(
            ...
            title: "",
            subtitle: viewModel.isNew ? "Thêm phiếu nhập" : (isEditMode ? "Chỉnh sửa phiếu nhập" : "Thông tin phiếu nhập"),
            isWhiteText: true,
            leading: {},
            trailing: {
                if isEditMode {
    ...
    @ViewBuilder
    private func profileHeaderCard(details: TPhieuvc_Nhap_DanhSach?) -> some View {
        VStack {
            HStack(spacing: 16) {
                ZStack {
                    Circle()
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text(viewModel.isNew ? "Phiếu nhập mới" : "Số: \(details?.soPhieu ?? "")")
                        .font(.title3.bold())
                        .foregroundColor(.white)
