...
struct PhieuGiaCongListView: View {
    ...
    private enum Strings {
        ...
        static let emptyTitle = "Không tìm thấy phiếu gia công"
        ...
        static func deleteAlertSubtitle(_ soPhieu: String) -> String { "Bạn có chắc chắn muốn xóa phiếu gia công \(soPhieu)?" }
        static let deleteBtn = "Xóa"
        static let cancelBtn = "Hủy"
        static func modalTitle(_ soPhieu: String) -> String { "Phiếu gia công: \(soPhieu)" }
        static let viewDetail = "Xem chi tiết"
        static let deleteTicket = "Xóa phiếu"
        static let navSubtitle = "Chuyển hàng gia công"
    ...
    @State private var selectedModalItem: TPhieuvc_Giacong_DanhSach? = nil
    
    init(fromDate: Date? = nil, toDate: Date? = nil, searchText: String? = nil) {
        let vm = PhieuGiaCongListViewModel(fromDate: fromDate, toDate: toDate, searchText: searchText)
        _viewModel = StateObject(wrappedValue: vm)
        _showSearchBar = State(initialValue: searchText != nil && !searchText!.isEmpty)
    }
    
    private var permission: TChucNangPhanQuyen? {
        AuthManager.shared.getPermission(for: "VTSSTAFF_DULIEU_PHIEUGIACONG")
    }
    ...
    private func deleteItem(_ item: TPhieuvc_Giacong_DanhSach) {
        router.showAlert(.alert, title: Strings.deleteAlertTitle, subtitle: Strings.deleteAlertSubtitle(item.soPhieu)) {
            Button(Strings.deleteBtn, role: .destructive) {
                Task {
                    do {
                        _ = try await PhieuGiaCongService.shared.xoa(soPhieu: item.soPhieu)
    ...
    @ViewBuilder
    private func cardItemView(item: TPhieuvc_Giacong_DanhSach) -> some View {
